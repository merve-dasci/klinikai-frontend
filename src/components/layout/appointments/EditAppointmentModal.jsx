...
function EditAppointmentModal({ isOpen, onClose, editingAppointment, onSubmit, patients, doctors }) {
  const {
    register,
    handleSubmit,
    reset,
  ...
  useEffect(() => {
    if (editingAppointment) {
      reset({
        ...
        patientId: editingAppointment.patientId ? String(editingAppointment.patientId) : "",
        doctorId: editingAppointment.doctorId ? String(editingAppointment.doctorId) : "",
      });
    }
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Edit Appointment" maxWidth="max-w-2xl">
